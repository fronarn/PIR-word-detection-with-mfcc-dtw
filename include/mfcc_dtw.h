...
mfcc_coefficients getMFCCcoefs(const paTestData data){
    ...
    float *dataFFT = new float[data.maxFrameIndex * NUM_CHANNELS] ;
    ...
    delete [] dataFFT; 
    dataFFT = new float[ (data.maxFrameIndex * NUM_CHANNELS) / 2 ] ;
    ...
    for (int i = 0; i < NB_COEF_MFCC; i++){
        coefs.coefs[i] = GetCoefficient(dataFFT, SAMPLE_RATE, NB_COEF_MFCC, (data.maxFrameIndex * NUM_CHANNELS) / 2, i);
        if ( i ){
